...
<body>
        ...
        <tbody>
            @forelse($tickets as $ticket)
                <tr>
                    ...
                    <td>
                        <strong>{{ $ticket->Subject ?? $ticket->subject }}</strong>
                        <div style="font-size: 11px; color:#777; margin-top:2px;">
                            {{ $ticket->category_name }} &bull; {{ $ticket->subcategory ?? 'General' }}
                        </div>
                    </td>
                    <td>
                        <span class="badge {{ strtolower($ticket->priority_name) }}">
                    ...
                    <td>
                        <span class="badge {{ strtolower(str_replace(' ', '-', $ticket->Status_name)) }}">
                            <i class="fa-solid fa-circle-dot"></i> {{ $ticket->Status_name }}
                        </span>
                    </td>
                    ...
                    <td>
    @if(str_contains(strtolower($ticket->Status_name), 'resolv'))
        <div style="display: flex; gap: 8px; align-items: center;">
            
          <form action="{{ url('/user/tickets/close/'.$ticket->Id) }}" method="POST" style="margin: 0;">
            @csrf
          ...
          <button type="button" class="badge open open-reopen-modal-btn" data-id="{{ Crypt::encryptString($ticket->Id) }}" style="background: #ce731e; border: 1px solid #9a240c; color: white; cursor: pointer; padding: 5px 10px;">
    ...
    @endif
       ...
       <td>
    <a href="{{ url('user/tickets/show/' . urlencode(Crypt::encryptString($ticket->Id ?? $ticket->Id))) }}" class="btn-show-details" style="display: inline-flex; align-items: center; gap: 6px; background-color: #0c369a; color: white; padding: 6px 12px; border-radius: 4px; font-size: 12px; text-decoration: none; font-weight: 500;">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (ticketStatusBtn) {
        ticketStatusBtn.addEventListener('click', function() {
            window.location.href = "{{ url('/ticket-status') }}";
        });
    }
